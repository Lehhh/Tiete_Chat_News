...
//}
    public ArrayList<String> getArrayBeginUrlNews() throws IOException {
        response = rq.executeGet(Strings.URL_CETESB_NEWS);

        for (i = 0; i < 3; i++) {

            inicio = response.indexOf("<a class=\"more-link\" href=\"");
            response = response.substring(inicio).replaceFirst("<a class=\"more-link\" href=\"", "");
            fim = response.indexOf("\">&nbsp;");
            String urlnews = response.substring(0, fim);
            urlNewsBegin.add(urlnews);
        }
...
//            System.out.println(urlNews.get(i));
    ...
    public ArrayList<String> getArrayCompleteUrlNews() throws IOException{
        ...
        for(i = 0; i< urlNewsCompleteAux.size(); i++){
            html = Strings.URL_CETESB +  urlNewsCompleteAux.get(i);
